...
if ( ! class_exists( 'Strong_Testimonials_Usage_Tracker' ) ) {
	 ...
	 */
	class Strong_Testimonials_Usage_Tracker {
		...
		public function __construct(
			...
			if ( basename( $this->plugin_file, '.php' ) != 'functions' ) {
				$this->plugin_name = 'strong-testimonials';
			}
		...
		public function get_data() {
			...
			$body['strong_testimonials_views']   = $this->get_views();
			$body['strong_testimonials_options'] = get_option( 'wpmtst_options' );
			// Return the data.
			return $body;
		...
		public function deactivate_this_plugin() {
			...
			// @since 1.2.2
			$track_time = get_option( 'strong_testimonials_wisdom_last_track_time' );
			if ( isset( $track_time[ $this->plugin_name ] ) ) {
				unset( $track_time[ $this->plugin_name ] );
			}
			update_option( 'strong_testimonials_wisdom_last_track_time', $track_time );
		...
		public function get_is_tracking_allowed() {
			...
			if ( $this->what_am_i == 'theme' ) {
				...
				$mod = get_theme_mod( 'strong_testimonials-wisdom-allow-tracking', 0 );
				if ( $mod ) {
					return true;
				}
			} else {
		...
		public function set_is_tracking_allowed( $is_allowed, $plugin = null ) {
			...
			$block_notice = get_option( 'strong_testimonials_wisdom_block_notice' );
			...
			if ( $this->has_user_opted_out() ) {
				if ( $this->what_am_i == 'theme' ) {
					set_theme_mod( 'strong_testimonials-wisdom-allow-tracking', 0 );
				} else {
				...
				if ( $this->what_am_i == 'theme' ) {
					set_theme_mod( 'strong_testimonials-wisdom-allow-tracking', 1 );
				} else {
		...
		public function get_is_time_to_track() {
			...
			$track_times = get_option( 'strong_testimonials_wisdom_last_track_time', array() );
		...
		public function set_track_time() {
			...
			update_option( 'strong_testimonials_wisdom_last_track_time', $track_times );
		}
		...
		public function set_notification_time() {
			$notification_times = get_option( 'strong_testimonials_wisdom_notification_times', array() );
			// Set different times according to plugin, in case we are tracking multiple plugins.
			if ( ! isset( $notification_times[ $this->plugin_name ] ) ) {
				...
				update_option( 'strong_testimonials_wisdom_notification_times', $notification_times );
			}
		}
		...
		public function update_block_notice( $plugin = null ) {
			...
			update_option( 'strong_testimonials_wisdom_block_notice', $block_notice );
		}
		...
		public function get_can_collect_email() {
			...
			$collect_email = get_option( 'strong_testimonials_wisdom_collect_email' );
		...
		public function set_can_collect_email( $can_collect, $plugin = null ) {
			...
			update_option( 'strong_testimonials_wisdom_collect_email', $collect_email );
		}
		...
		public function get_admin_email() {
			// The wisdom_collect_email option is an array of plugins that are being tracked.
			$email = get_option( 'strong_testimonials_wisdom_admin_emails' );
			// If this plugin is in the array, then we can collect the email address.
		...
		public function set_admin_email( $email = null, $plugin = null ) {
			...
			update_option( 'strong_testimonials_wisdom_admin_emails', $admin_emails );
		}
		...
		public function get_views() {
			...
			$license_key = get_option( 'strong_testimonials_pro_license_key', false );
			...
			foreach ( $views as $view ) {
				...
				if ( isset( $view['name'] ) && strlen( $view['name'] ) != 0 ) {
					$view[ $id ]['name'] = $view['name'];
				}

				$return_array[ $id ]['license']  = $license_key;
				$return_array[ $id ]['site_url'] = $site_url;
				$return_array[ $id ]['id']       = $id;
				$return_array[ $id ]['settings'] = $view['data'];
